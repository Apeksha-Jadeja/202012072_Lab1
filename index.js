...
app.get('/voteresults', (req, res) => {
    res.render('voteresult', { list: listcandidates });
});

app.post('/countvote', (req, res) => {
    if (votedlist.has(req.body.id)) {
        res.render('vote', { list: listcandidates, message: "You have already voted, ThankYou" });
    } else {
        ...
        res.render('vote', { list: listcandidates, message: "ThankYou" });
    }
});
